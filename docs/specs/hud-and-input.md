...
- **REQ-HUD-001** — `HUDScene` shall display: an HP bar (colour shifts green→amber→red as HP drops)
  ...
  counter, the run score, the combo readout, and a run-lives readout (hearts, top-right) fed by
  `lives-update` / `GameScene.livesRemaining()` and hidden during the tutorial.
